...
float getSplattedTexture(in sampler3D texture_toSplat, in vec3 coord, in vec3 step_size);
...
void main() {
	...
	{
		float splat = getSplattedTexture(astro_tex, G_fragTexCoord, vec3(0.001,0.001,0.002));
	...
	{
		float splat = getSplattedTexture(mito_tex, G_fragTexCoord, vec3(0.001, 0.001, 0.002));
	...
	//{
	//float splat = getSplattedTexture(gly_tex, G_fragTexCoord, vec3(0.015625));
